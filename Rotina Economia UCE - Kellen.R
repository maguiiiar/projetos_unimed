...
colnames(custo.drg.2016)[colnames(custo.drg.2016)=="Codigo Registro Paciente"] <- "Identificador do Paciente"
colnames(custo.drg.2017)[colnames(custo.drg.2017)=="Codigo Registro Paciente"] <- "Identificador do Paciente"

custo.drg.2016.soma <- custo.drg.2016 %>% group_by(`Identificador do Paciente`) %>% summarise(Custo = sum(`Custo Total (R$)`)); rm(custo.drg.2016)
custo.drg.2017.soma <- custo.drg.2017 %>% group_by(`Identificador do Paciente`) %>% summarise(Custo = sum(`Custo Total (R$)`)); rm(custo.drg.2017)
custo.drg.soma <- bind_rows(custo.drg.2016.soma, custo.drg.2017.soma)

dados.drg.2016 <- left_join(dados.drg.2016, custo.drg.2016.soma, by = "Identificador do Paciente")
dados.drg.2017 <- left_join(dados.drg.2017, custo.drg.2017.soma, by = "Identificador do Paciente")
dados.drg.2016$Percentil = as.character(dados.drg.2016$Percentil)
...
dados.drg <- bind_rows(dados.drg.2016, dados.drg.2017); rm(dados.drg.2016, dados.drg.2017)
...
#dados.drg <- dados.drg %>% filter(as.Date(`Data de Internação`) < "2017-11-01")

teste <- dados.drg %>% filter(`Código do Paciente` %in% c("141211108363002", "141155902179005", 
...
benef.UCE.2016 <- dados.drg.2016 %>% filter(`Código do Paciente` %in% c("141211108363002", "141155902179005", 
                                                                         ...
                                                                         "140713376127013", "140706188731003", 
                                                                         "145010312366007", "142103980943004")) %>%
                                        group_by(`Identificador do Paciente`, `Código do Paciente`, `Nome do Paciente`, `Data de Internação`, `Tipo de DRG`) %>%
                                        summarise(`Permanência Real` = sum(`Permanência Real`),
                                                  ...
                                                  `Custo` = sum(`Custo`),
                                                  `Internações Distintas` = n_distinct(`Identificador do Paciente`))
...
benef.UCE.2017 <- dados.drg.2017 %>% filter(`Código do Paciente` %in% c("141211108363002", "141155902179005", 
                                                                       ...
                                                                       "140713376127013", "140706188731003", 
                                                                       "145010312366007", "142103980943004")) %>%
                                                      group_by(`Identificador do Paciente`, `Código do Paciente`, `Nome do Paciente`, `Data de Internação`, `Tipo de DRG`, `Paciente Internado Outras Vezes`) %>%
                                                      summarise(`Permanência Real` = sum(`Permanência Real`),
                                                                `Permanência Prevista na Alta` = sum(`Permanência Prevista na Alta`), 
                                                                `Permanência Prevista Internação` = sum(`Permanência Prevista Internação`),
                                                                `Custo` = sum(`Custo`),
...
benef.UCE <- bind_rows(benef.UCE.2016, benef.UCE.2017)

write.csv(teste, "TESTE.csv")

benef.UCE <- benef.UCE %>% group_by(`Tipo de DRG`) %>% summarise(`Permanência Real` = sum(`Permanência Real`),
                                                                 `Internações Distintas` = sum(`Internações Distintas`),
                                                                 `Tempo Médio` = `Permanência Real`/`Internações Distintas`)
...
dados.drg.uti <- fread("DRG UTI - 2016 e 2017.txt", sep = ";", header = TRUE, dec = ",", na.strings = c("", NA))
dados.drg.uti <- left_join(dados.drg.uti, custo.drg.soma, by = "Identificador do Paciente")
dados.drg.uti$`Código do Paciente` = as.character(dados.drg.uti$`Código do Paciente`)
dados.drg.uti.soma <- dados.drg.uti %>% filter(`Código do Paciente` %in% c("141211108363002", "141155902179005", 
                                                                        "141111501061001", "142328002052506", 
                                                                        "141101012048005", "141113905008010", 
                                                                        "145010341206012", "140731743453006", 
                                                                        "145010313101007", "142783000004006", 
...
write.csv(dados.drg.uti, file="DRG UTI.csv")

tapply(teste$`Permanência Real`, teste$`Tipo de DRG`, FUN=geometric.mean, na.rm = TRUE)
