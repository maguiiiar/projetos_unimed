...
base.pj.uu <- fread("Base contratos Pre Pgto P.J 06 09 2018.txt", sep = "\t", colClasses = c("Cod Contrato Ajustado" = "character"))
...
vidas <- base.pj.uu %>% group_by(`Cod Contrato Ajustado`) %>% summarise(Vidas = sum(Vidas, na.rm = TRUE))
`Cadastro PJ e CNAE` <- `Cadastro PJ e CNAE` %>% select(-c(Vidas, `Cod Contrato`, ContratoChave)) %>% distinct(.)

`Cadastro PJ e CNAE` <- left_join(`Cadastro PJ e CNAE`, vidas, by = "Cod Contrato Ajustado")
...
base.despesas.pj.uu.melt <- base.despesas.pj.uu.melt %>% group_by(`Contrato Codigo`, `Contrato Nome`) %>% summarise(Despesa = sum(Despesa, na.rm = TRUE))
base.receitas.pj.uu.melt <- base.receitas.pj.uu.melt %>% group_by(`Contrato Codigo`, `Contrato Nome`) %>% summarise(Receita = sum(Receita, na.rm = TRUE)) 
base.despesas.receitas.pj.uu <- full_join(base.despesas.pj.uu.melt, base.receitas.pj.uu.melt); names(base.despesas.receitas.pj.uu)[1] <- "Cod Contrato Ajustado"
base.cnpj.pj <- `Cadastro PJ e CNAE` %>% select(`CNPJ`,`Cod Contrato Ajustado`)
base.despesas.receitas.pj.uu <- left_join(base.despesas.receitas.pj.uu, base.cnpj.pj, by = "Cod Contrato Ajustado")
